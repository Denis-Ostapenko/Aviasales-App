...
const Ticket = (props) => {
  ...
  const { origin: thereOrigin, destination: thereDestination, date: thereDate, stops: thereStops, duration: thereDuration } = segments[0];
  const { origin: backOrigin, destination: backDestination, date: backDate, stops: backStops, duration: backDuration } = segments[1];
  ...
  const priceTravel = (cost) => {
    const strPrice = `${cost}`.split('').reverse().map((item, i) => {
      if (i === 2) {
        return ` ${item}`
      }
      return item
    ...
    <div className={classes.ticket}>
      ...
      <main>
        <div className={classes.ticket__element}>
          <div className='route'>
            <span>{thereOrigin} – {thereDestination}</span>
        ...
        <div className={classes.ticket__element}>
          <div className='route'>
            <span>{backOrigin} – {backDestination}</span>
